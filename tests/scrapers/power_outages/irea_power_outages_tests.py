...
from event_indexing.scrapers.power_outages.irea_power_outages import IREAPowerOutages
from event_indexing.source import TYPE_CAD_API
from tests.scrapers.power_outages import get_data_path


class IREAPowerOutagesTest(unittest.TestCase):
    def setUp(self):
        with open(get_data_path('irea_power_outages.xml')) as f:
            html = f.read()

        with open(get_data_path('irea_power_outages.json')) as f:
            data = json.load(f)
        ...
        self.scraper = IREAPowerOutages(self.relay_host_api, self.relay_auth, self.proxy_host)

    def test_name(self):
        self.assertEqual(self.scraper.name, 'IREAPowerOutages')

    def test_tz_name(self):
        self.assertEqual(self.scraper.tz_name, 'US/Mountain')
    ...
    def test_get_request_args(self):
        ...
        self.assertEqual(url, 'http://irea.maps.sienatech.com/data/outages.xml')
    ...
    @patch('event_indexing.scrapers.power_outages.irea_power_outages.get_tz_now')
    @patch('time.time', return_value=1471568199)
