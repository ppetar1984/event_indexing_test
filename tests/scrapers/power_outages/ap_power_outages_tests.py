...
from event_indexing.scrapers.power_outages.ap_power_outages import APPowerOutages
from event_indexing.source import TYPE_CAD_API
from tests.scrapers.power_outages import get_data_path


class APPowerOutagesTest(unittest.TestCase):
    def setUp(self):
        with open(get_data_path('ap_power_outages.json')) as f:
            data = json.load(f)
        ...
        self.scraper = APPowerOutages(self.relay_host_api, self.relay_auth, self.proxy_host)

    def test_name(self):
        self.assertEqual(self.scraper.name, 'APPowerOutages')

    def test_tz_name(self):
        self.assertIsNone(self.scraper.tz_name)

    def test_get_url(self):
        url = self.scraper.get_url(directory='2017_03_18_13_45_30', index='0320012332')

        self.assertEqual(url,
                         'http://outagemap.appalachianpower.com.s3.amazonaws.com/resources/data/external/interval_generation_data/2017_03_18_13_45_30/outages/0320012332.json')
    ...
    def test_scrape(self, mock_time):
        ...
        self.assertEqual(incident_id, 'b1e03f2684032b3667a238ccb0942737')
        self.assertEqual(latitude, 36.67027)
        self.assertEqual(longitude, -79.79946)
    ...
    def test_parse_alert(self):
        ...
        self.assertEqual(incident, 'Power Outage')
        self.assertEqual(latitude, 36.70398)
        self.assertEqual(longitude, -82.00351)
    ...
    def test_parse_alert_source(self):
        ...
        self.assertEqual(created_at, 1471567800.0)
        self.assertEqual(id, 'b1e03f2684032b3667a238ccb0942737')
        self.assertEqual(text, 'Power Outage')
        self.assertEqual(source_type, TYPE_CAD_API)
        self.assertEqual(coordinates, [-82.00351, 36.70398])
        self.assertEqual(geo_type, 'Point')

        self.assertEqual(provider_api_host, 'outagemap.appalachianpower.com.s3.amazonaws.com')
        self.assertEqual(provider_api_route,
                         '/resources/data/external/interval_generation_data/2017_03_18_13_45_30/outages/0320012332.json')
        self.assertEqual(provider_api_name, 'Appalachian American Electric Power Co')
        self.assertEqual(provider_api_id, 'ap_power_outages')
    ...
    def test_run(self, mock_requests, mock_time):
        ...
        APPowerOutages._indexes = [self.data['indexes']]
        APPowerOutages._directory = meta['directory']
    ...
    def test_get_description(self):
        ...
        raw_incident['desc']['cause'] = 'Customer Problem'
        description = self.scraper.get_description(raw_incident)

        self.assertEqual(description, 'Customer Problem')
